...
let createSearchEngine = function () {
	...
	function score(length, distance, count, docs) {
		let relevance =
			(Math.min(1.0, 1.0 / (docs + 3)) * length) /
			(length + 8 * distance);
	...
	module.add = function (id, content) {
		if (module.doclength.hasOwnProperty(id))
			throw "[searchengine] document ID is already in use";
		let tokens = module.tokenize(content);
		module.doclength[id] = tokens.length;
		for (let i = 0; i < tokens.length; i++) {
			let t = tokens[i];
			if (!module.dictionary.hasOwnProperty(t))
				module.dictionary[t] = { docs: {}, numdocs: 0 };
	...
	module.find = function (keys, n = 10) {
		...
		for (let i = 0; i < keys.length; i++) {
			...
			for (let word in module.dictionary) {
				...
				let threshold = Math.floor(
					module.config.maxreldist * word.length
				);
				...
				for (let id in worddata.docs) {
					...
					if (!resultdict.hasOwnProperty(id))
						resultdict[id] = { id: id, score: 0.0, matches: {} };
					let s = score(word.length, dist, worddata.docs[id], num);
					resultdict[id].score += s;
					if (!resultdict[id].matches.hasOwnProperty(word))
						resultdict[id].matches[word] = 0.0;
		...
		for (let id in resultdict) {
			...
			let result: any = resultdict[id];
			result.score =
				Math.log(result.score) / Math.log(module.doclength[id]);
			resultarray.push(result);
		}

		// return the best n results
		resultarray.sort(function (lhs, rhs) {
...
export const searchengine = createSearchEngine(); // default instance

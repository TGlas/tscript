...
export const evaluation = (function () {
	...
	module.hasStructure = function (program, pseudo) {
		...
		function isSubtree(key, data) {
			// The key matches the data if
			//  * isObject(key):
			//     - the data object matches the key (petype, optionally base, optionally name), and sub-items match sub-items
			//     - any child matches the key, recursively (array items and object attributes, irrespective of the name)
			//  * Array.isArray(key):
			//     - isObject(data): any data object attribute matches the key
			//     - Array.isArray(data):
			//       - a single array item matches the whole key
			//       - each data item matches one key item, in the correct order
			if (!Array.isArray(data) && !isObject(data)) return false;
			if (isObject(key)) {
				if (isObject(data)) {
					if (data.builtin) return false;
					if (data.hasOwnProperty("petype")) {
						let match = data.petype === key.petype;
						if (match && key.hasOwnProperty("base")) {
							if (data.base !== key.base) match = false;
						}
						if (match && key.hasOwnProperty("name")) {
							if (data.name !== key.name) match = false;
						}
						if (match && key.hasOwnProperty("sub")) {
							let match_sub = false;
							for (let name in data) {
								if (name in ignore) continue;
								if (isSubtree(key.sub, data[name])) {
									match_sub = true;
									break;
								}
							}
							if (!match_sub) match = false;
						}
						if (match) return true;
					}
					for (let name in data) {
						if (name in ignore) continue;
						if (isSubtree(key, data[name])) return true;
					}
				} else if (Array.isArray(data)) {
					for (let i = 0; i < data.length; i++)
						if (isSubtree(key, data[i])) return true;
				}
				...
				if (isObject(data)) {
					...
					// try to match items one by one, possibly with gaps
					let j = 0,
						n = key.length;
					for (let i = 0; i < data.length; i++) {
						if (isSubtree(key[j], data[i])) {
							j++;
							if (j === n) return true;
						}
					}

					// try to match the whole key with a single data item
					for (let i = 0; i < data.length; i++)
						if (isSubtree(key, data[i])) return true;
				}
			} else
				throw new Error("[hasStructure] internal error in isSubtree");
			return false;
		}
		...
		console.log("[compilePseudo]", pseudo, pc);
		if (pc === null) {

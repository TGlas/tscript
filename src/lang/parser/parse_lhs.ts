	...
	if (ex.petype === "name") {
		...
		ex.step = function () {
			...
			if (ip === 0) {
				...
				let key;
				if (
					TScript.isDerivedFrom(container.type, Typeid.typeid_string)
				) {
					state.error("/argument-mismatch/am-32", ["a substring"]);
				} else if (
					TScript.isDerivedFrom(container.type, Typeid.typeid_array)
				) {
					if (
						TScript.isDerivedFrom(index.type, Typeid.typeid_integer)
					) {
						if (index.value.b < 0)
					...
					if (
						!TScript.isDerivedFrom(index.type, Typeid.typeid_string)
					)
					...
					state.error("/argument-mismatch/am-31b", [container.type]);

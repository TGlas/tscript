...
export function parse_expression(
			...
			ex.passResolve = (function (ex) {
				return function (state) {
					...
					if (ex.hasOwnProperty("super")) {
						...
						ex.reference = resolve_name(state, ex.name, cls.superclass, "expression").names[
							ex.name
						];

						// check private access modifier
						if (ex.reference.hasOwnProperty("access") && ex.reference.access === "private")
						...
						ex.reference = resolve_name(state, ex.name, ex.parent, "expression").names[ex.name];

					// check whether "this" is available
					if (ex.reference.petype === "attribute" || ex.reference.petype === "method") {
						...
						if (fn && fn.petype !== "method")
							state.error("/syntax/se-13", ["expression", ex.reference.petype, ex.name]);
					}
					...
					if (ex.reference.petype === "variable" || ex.reference.petype === "attribute") {
						...
						ex.step = function () {
							...
							let tv: any = null;
							if (pe.scope === "global") tv = this.stack[0].variables[pe.id];
							else if (pe.scope === "local") tv = frame.variables[pe.id];
							else if (pe.scope === "object") tv = frame.object.value.a[pe.id];
							else ErrorHelper.assert(false, "unknown scope: " + pe.scope);

							if (tv && tv?.type && tv?.value) frame.temporaries.push(tv);
							else this.error("/name/ne-29", [TScript.displayname(pe)]);
						...
						ex.petype = "constant";
						ex.typedvalue = {
							type: get_program(ex.parent).types[Typeid.typeid_function],
						...
						ex.petype = "constant";
						ex.typedvalue = {
							type: get_program(ex.parent).types[Typeid.typeid_type],
							value: { b: ex.reference },
						};
						ex.step = constantstep;
						ex.sim = simfalse;
					} else if (ex.reference.petype === "namespace" && allow_namespace) {
					} else {
			...
			ex.passResolveBack = (function (ex) {
				return function (state) {
					if (ex.petype == "non-value name") {
						state.set(ex.where);
						if (ex.reference.petype === "namespace") state.error("/name/ne-11");
				...
				let op: any = {
					...
					step: function () {
						...
						if (ip === 0) {
							...
							if (TScript.isDerivedFrom(object.type, Typeid.typeid_type)) {
								...
								if (m === null)
									state.error("/name/ne-12", [TScript.displayname(type), pe.member]);
							} else {
								// non-static case
								...
								if (m === null)
									state.error("/name/ne-13", [TScript.displayname(type), pe.member]);
							}
							...
							if (m.petype === "method") {
								...
								if (tv && tv?.type && tv?.value) frame.temporaries.push(tv);
								else this.error("/name/ne-29", [TScript.displayname(m)]);
					...
					passResolve: function (state) {
						...
						if (
							...
							// resolve statically
							let reference = op.object.reference.names[op.member];
							if (reference) {
								...
								if (
									...
									state.error("/name/ne-5", ["expression", op.name]);
								}

								// create the "name" object
								if (reference.petype === "variable" || reference.petype === "attribute") {
									...
									op.step = constantstep;
									op.sim = simfalse;
								} else if (reference.petype === "namespace" && allow_namespace) {
								} else {
									ex.petype = "non-value name"; // this name must be qualified further to yield a value
								}
							} else {
								state.error("/name/ne-5", ["expression", op.member]);
							}
						}
					},
						...
						if (ip === 0) {
							...
							if (TScript.isDerivedFrom(container.type, Typeid.typeid_string)) {
								...
								if (TScript.isDerivedFrom(index.type, Typeid.typeid_string)) {
									if (container.value.b.hasOwnProperty("#" + index.value.b))
										frame.temporaries.push(container.value.b["#" + index.value.b]);
								...
								if (TScript.isDerivedFrom(index.type, Typeid.typeid_integer)) {
									...
									if (index.value.b < 0 || index.value.b >= len)
										state.error("/argument-mismatch/am-29", [
											TScript.toString.call(this, index),
											len,
										]);
									let ret = {
										type: this.program.types[Typeid.typeid_integer],
										value: {
											b: container.value.b.begin + index.value.b,
										},
									};
									frame.temporaries.push(ret);
								} else if (TScript.isDerivedFrom(index.type, Typeid.typeid_range)) {
									...
									let ret = {
										...
										value: {
											b: {
												begin: container.value.b.begin + a,
												end: container.value.b.begin + b,
											},
										},
									};
									frame.temporaries.push(ret);
	...
	let processUnary = function (j) {
		...
		ErrorHelper.assert(
			...
			"[processUnary] cannot find left-unary operator " + stack[j].operator
		);
	...
	if (lhs) {
		if (stack[0].petype === "group")
			state.error("/argument-mismatch/am-32", ["expression in parentheses"]);
		...
		if (stack[0].petype === "function call")
			state.error("/argument-mismatch/am-32", ["the result of a function call"]);
	}

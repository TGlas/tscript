...
export class ErrorHelper {
	...
	public static errorTemplate(path): String {
		...
		ErrorHelper.assert(tokens[0] === "", "[getError] invalid path: " + path);
		let ret = errors;
		for (let i = 1; i < tokens.length; i++) {
			ErrorHelper.assert(ret.hasOwnProperty(tokens[i]), "[getError] invalid path: " + path);
			ret = ret[tokens[i]];
		}
	...
	public static assert(condition: any, message: string | undefined = undefined): void {
...
export let errors = {
	syntax: {
		...
		"se-43": "syntax error in member access: identifier expected after the dot '.'",

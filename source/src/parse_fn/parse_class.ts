...
export function parse_class(state, parent)
{
	...
	// check class name
	if (TScript.options.checkstyle && ! state.builtin() && (cname[0] < 'A' || cname[0] > 'Z'))
	{
	...
	while (true)
	{
		...
		else if (token.type === "keyword" && token.value === "constructor")
		{
			...
			let pe:any = parse_constructor(state, cls);
			pe.access = access;
		...
		else if (token.type === "keyword" && token.value === "class")
		{
			...
			let pe:any = parse_class(state, cls);
			pe.displayname = cname + "." + pe.name;
			pe.access = access;
			cls.staticmembers[pe.name] = pe;
		}

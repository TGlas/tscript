...
export function parse_trycatch(state, parent)
{
	// handle "try" keyword
	let where = state.get();
	let token = Lexer.get_token(state);
	ErrorHelper.assert(token.type === "keyword" && token.value === "try", "[parse_trycatch] internal error");
	...
	let pe:any = { "petype": "variable", "where": where_var, "parent": trycatch.catch_part, "name": name, "id": id, "scope": "local" };

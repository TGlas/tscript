...
import { get_function } from "../runner_helper";
...
export function parse_for(state, parent)
{
	// handle "for" keyword
	let where = state.get();
			...
			"step": function()
					{
						...
						else if (ip === 1)
						{
							...
							let container:any = TScript.isDerivedFrom(iterable.type,Typeid.typeid_range) ? { "rangemarker": null, "begin": iterable.value.b.begin, "end": iterable.value.b.end, "length": Math.max(0, iterable.value.b.end - iterable.value.b.begin) } : new Array();
						...
						else if (ip === 2)
						{
							...
							else if (pe.hasOwnProperty("var_id"))
							{
								let typedvalue = container.hasOwnProperty("rangemarker")
										? {"type": this.program.types[Typeid.typeid_integer], "value": {"b": (container.begin + index) | 0}}
										: container[index];
	...
	if (token.type === "keyword" && token.value === "var")
	{
		...
		let pe:any = { "petype": "variable", "where": where, "parent": forloop, "name": token.value, "id": id, "scope": "local" };
	...
	else
	{
		...
		if (token.type === "identifier" && token.value === "in")
		{
			...
			// parse "in"
			token = Lexer.get_token(state);
			ErrorHelper.assert(token.type === "identifier" && token.value === "in", "[parse_for] internal error");

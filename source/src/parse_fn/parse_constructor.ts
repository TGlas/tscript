...
import { constantstep, get_program } from "../runner_helper";
...
export function parse_constructor(state, parent)
{
	// check that the parent is indeed a type
	ErrorHelper.assert(parent.petype === "type", "[parse_constructor] internal error; parent is expected to be a type");

	// handle "constructor" keyword
	let where = state.get();
	let token = Lexer.get_token(state);
	ErrorHelper.assert(token.type === "keyword" && token.value === "constructor", "[parse_constructor] internal error");

	// create the function
	let func:any = { "petype": "method", "where": where, "declaration": true, "parent": parent, "commands": new Array(), "variables": new Array(), "name": "constructor", "names": {}, "params": new Array(), "step": constructorstep, "sim": simfalse };
	...
	if (parent.hasOwnProperty("superclass"))
	{
		...
		let base = {"petype": "constant",
		            "where": state.get(),
		            "typedvalue": {"type": get_program(parent).types[Typeid.typeid_type], "value": {"b": parent.superclass}},
		...
		else 
		{
			...
			func.supercall = { "petype": "super call", "where": where, "parent": func, "base": base, "arguments": new Array(), "step": callstep, "sim": callsim };
		}
	}

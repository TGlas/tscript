...
import { asConstant, constantstep, get_function, get_program, scopestep } from "../runner_helper";
...
export function parse_expression(state, parent, lhs:any | undefined = undefined)
{
	...
	while (true)
	{
		...
		else if (token.value === '[')
		{
			...
			while (! state.eof())
			{
				...
				if (token.type === "grouping" && token.value === ']') { Lexer.get_token(state); break; }

...
import { get_type, copyconstant } from "../runner_helper";
...
export function parse_call(state, parent, base)
{
	...
	if (token.type === "grouping" && token.value === ')') Lexer.get_token(state);
	else
	{
		...
		while (true)
		{
			...
			let where = state.get();
			let named = false;
			let name = Lexer.get_token(state);

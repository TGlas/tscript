...
export function parse_function(state, parent, petype:any = undefined)
{
	...
	ErrorHelper.assert(token.type === "keyword" && token.value === "function", "[parse_function] internal error");
	...
	// check function name
	if (TScript.options.checkstyle && ! state.builtin() && fname[0] >= 'A' && fname[0] <= 'Z')
	{
		state.error("/style/ste-3", ["function", fname]);
	}

	// create the function
	let func:any = { "petype": petype, "where": where, "declaration": true, "parent": parent, "commands": new Array(), "variables": new Array(), "name": fname, "names": {}, "params": new Array(), "step": scopestep, "sim": simfalse };

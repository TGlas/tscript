...
// Recursively apply an operation recursively to a loop-free typed data structure.
...
// The function compose is invoked on a container holding return values of operation and container.
// Both functions return the processed result.
...
// error messages
//
// Here comes a nested dictionary containing all error
// messages, as well as a few helper functions for error
// processing and display. An error is identified by an ID,
// which is an absolute path into the nested dictionary.
//
...
let core = {
	...
	"impl": {
			...
			"exists": function(key) {
				...
				let ret = localStorage.getItem("tscript.data." + key) !== null;
				return {"type": this.program.types[module.typeid_boolean], "value": {"b": ret}};
			},
			"load": function(key) {
	...
	"source":
		...
		"impl": {
			"math": {
					...
					"min": function(a, b) {
						if (module.order(a, b) <= 0) return a; else return b;
					},
					"max": function(a, b) {
						if (module.order(a, b) >= 0) return a; else return b;
					},
	...
	"impl": {
			"turtle": {
					"reset": function(x, y, degrees, down) {
						...
						{
							this.service.turtle.turtle_position = [x.value.b, y.value.b];
							this.service.turtle.turtle_angle = degrees.value.b;
							this.service.turtle.turtle_pen = down.value.b;
							this.service.turtle.turtle_color = "rgb(0,0,0)";
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					"move": function(distance) {
						...
						if (this.service.turtle)
						{
							let x = this.service.turtle.turtle_position[0];
							let y = this.service.turtle.turtle_position[1];
							let a = Math.PI / 180 * this.service.turtle.turtle_angle;
							let s = Math.sin(a);
							let c = Math.cos(a);
							let xx = x + s * distance.value.b;
							let yy = y + c * distance.value.b;
							this.service.turtle.turtle_position = [xx, yy];
							if (this.service.turtle.turtle_pen)
							{
								let ctx = this.service.turtle.getContext("2d");
								ctx.lineWidth = 1;
								ctx.strokeStyle = this.service.turtle.turtle_color;
								ctx.beginPath();
								ctx.moveTo(300+3*x, 300-3*y);
								ctx.lineTo(300+3*xx, 300-3*yy);
								ctx.stroke();
							}
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					...
					"color": function(red, green, blue) {
						...
						if (this.service.turtle) this.service.turtle.turtle_color = "rgb(" + Math.round(255*r) + "," + Math.round(255*g) + "," + Math.round(255*b) + ")";
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					"turn": function(degrees) {
						...
						if (this.service.turtle) this.service.turtle.turtle_angle = (this.service.turtle.turtle_angle + degrees.value.b) % 360.0;
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					"pen": function(down) {
						...
						if (this.service.turtle) this.service.turtle.turtle_pen = down.value.b;
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
			},
		},
...
let lib_canvas = {
	...
	"impl": {
			"canvas": {
					"width": function() {
						let ret = 0;
						if (this.service.canvas) ret = this.service.canvas.width;
						return {"type": this.program.types[module.typeid_integer], "value": {"b": ret}};
					},
					"height": function() {
						let ret = 0;
						if (this.service.canvas) ret = this.service.canvas.height;
						return {"type": this.program.types[module.typeid_integer], "value": {"b": ret}};
					},
					"setLineWidth": function(width) {
						...
						if (this.service.canvas)
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.lineWidth = width.value.b;
						}
					},
					"setLineColor": function(red, green, blue, alpha) {
						...
						{
							let ctx = this.service.canvas.getContext("2d");
							let r = Math.min(1, Math.max(0, red.value.b));
							let g = Math.min(1, Math.max(0, green.value.b));
							let b = Math.min(1, Math.max(0, blue.value.b));
							let a = Math.min(1, Math.max(0, alpha.value.b));
					...
					"setFont": function(fontface, fontsize) {
						...
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.font = fontsize.value.b + "px " + fontface.value.b;
							this.service.canvas.font_size = fontsize.value.b;
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					...
					"line": function(x1, y1, x2, y2) {
						...
						if (this.service.canvas)
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.beginPath();
							ctx.moveTo(x1.value.b, y1.value.b);
							ctx.lineTo(x2.value.b, y2.value.b);
							ctx.stroke();
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					"rect": function(left, top, width, height) {
						...
						if (this.service.canvas)
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.beginPath();
							ctx.rect(left.value.b, top.value.b, width.value.b, height.value.b);
							ctx.stroke();
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					"fillRect": function(left, top, width, height) {
						...
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.fillRect(left.value.b, top.value.b, width.value.b, height.value.b);
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					...
					"circle": function(x, y, radius) {
						...
						if (this.service.canvas)
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.beginPath();
							ctx.arc(x.value.b, y.value.b, radius.value.b, 0, 2 * Math.PI, false);
							ctx.stroke();
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					...
					"curve": function(points, closed) {
						...
						if (this.service.canvas)
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.beginPath();
							let n = points.value.b.length;
							if (n > 0)
							{
								let p = points.value.b[0];
								ctx.moveTo(p.value.b[0].value.b, p.value.b[1].value.b);
								for (let i=1; i<n; i++)
								{
									let p = points.value.b[i];
									ctx.lineTo(p.value.b[0].value.b, p.value.b[1].value.b);
								}
							}
							if (closed.value.b) ctx.closePath();
							ctx.stroke();
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					...
					"text": function(x, y, str) {
						...
						if (this.service.canvas)
						{
							...
							let lines = module.toString(str).split('\n');
							let xx = x.value.b;
							let yy = y.value.b;
							for (let i=0; i<lines.length; i++)
							{
								ctx.fillText(lines[i], xx, yy);
								yy += this.service.canvas.font_size;
							}
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					...
					"shift": function(dx, dy) {
						...
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.translate(dx.value.b, dy.value.b);
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
					"scale": function(factor) {
						...
						{
							let ctx = this.service.canvas.getContext("2d");
							ctx.scale(factor.value.b, factor.value.b);
						}
						return {"type": this.program.types[module.typeid_null], "value": {"b": null}};
					},
...
// string. It handles composite operators properly.
//
...
// into a description object. Each object in the structure
// shall provide the following fields:
...
// the exception is that it interrupts the control flow;
// the actual error message is stored in the state object.
//

// deep copy of a JSON-style data structure
...
//		if (sub_cl === null || ! module.isDerivedFrom(sub_cl, super_cl.id)) state.error("/syntax/se-12", [errorname, lookup.petype, name, module.displayname(super_cl)]);
...
function parse_call(state, parent, base)
{
	...
	if (base.petype == "constant")
	{
		...
		{
			var cls = base.typedvalue.value.b;
			...
			"step": function()
					{
						...
						else
						{
							...
							let arg = frame.temporaries.pop();
//							module.assert(frame.temporaries.length == 0, "temporaries stack is non-empty when returning from a function");
							this.stack.pop();
...
// running in the background, then it can also be run in step-by-step in

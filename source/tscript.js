...
//inject a polyfill for window.requestIdleCallback()
		...
		setTimeout(function(){
			let deadline = {
				...
				timeRemaining: function(){
					return this.startTime + this.maxTime - Date.now();
				}
			};
			callback(deadline);
		}, 1);
...
module.Interpreter = function(program)
{
	...
	this.chunk = function(deadline)
	{
		...
		if (this.status === "running")
		{
			...
			while (this.background && deadline.timeRemaining() > 3)
			{
				this.exec_step();
		...
		if(this.status !== "finished"){
			window.requestIdleCallback(this.chunk.bind(this));
		}

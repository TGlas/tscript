...
let lib_canvas = {
	...
	// moves data from a tscript array of reals into a Float32 array; returns false if the buffer contains invalid data
	let fillAudioBuffer = function(tscriptBuffer, array)
	{
		...
		for (let i=0; i<array.length; i++)
		{
			...
			if (! module.isDerivedFrom(tscriptBuffer.value.b[i].type, module.typeid_real)) return false;
			...
			"audio":{
				"MonoAudio":{
					"constructor":function(object, buffer, sampleRate)
					{
						...
						if (! hasAudioContext.bind(this)()) return;
						let buf = this.service.audioContext.createBuffer(1, buffer.value.b.length, sampleRate.value.b);

						if (! fillAudioBuffer(buffer, buf.getChannelData(0))) this.error("/argument-mismatch/am-44", ["buffer", "real"]);
				...
				"StereoAudio":{
					"constructor":function(object, leftBuffer, rightBuffer, sampleRate)
					{
						...
						if (! fillAudioBuffer(leftBuffer, buf.getChannelData(0))) this.error("/argument-mismatch/am-44", ["leftBuffer", "real"]);
						if (! fillAudioBuffer(rightBuffer, buf.getChannelData(1))) this.error("/argument-mismatch/am-44", ["rightBuffer", "real"]);
